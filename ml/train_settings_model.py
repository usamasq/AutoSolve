...
def generate_fallback_weights() -> dict:
    ...
    # 29 input features, 64 hidden, 32 hidden, 1 output
...
def train_model(args):
    ...
    train_X = torch.tensor(dataset["train"]["X"], dtype=torch.float32).to(device)
    train_y = torch.tensor(dataset["train"]["y"], dtype=torch.float32).unsqueeze(1).to(device)
    val_X = torch.tensor(dataset["val"]["X"], dtype=torch.float32).to(device)
    val_y = torch.tensor(dataset["val"]["y"], dtype=torch.float32).unsqueeze(1).to(device)
            ...
            try:
                if args.pretrained.endswith('.json'):
                    ...
                    for k, v in meta_data["weights"].items():
                        weights_dict[k] = torch.tensor(v, dtype=torch.float32).to(device)
    ...
    print(f"Training complete. Best Validation SmoothL1 Loss: {best_val_loss:.6f}")
