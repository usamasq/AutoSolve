...
class FilteringMixin:
    ...
    def filter_high_error(self, max_error: float = 3.0):
        ...
        def covers_keyframes(track):
            ...
            # Optimization: Use O(1) C-lookup instead of iterating markers O(M)
        ...
        # Optimization: membership check in set is O(1)
        ...
        if len(critical_candidates) > max_critical_loss:
            ...
            critical_errors = []
            # Optimization: Iterate candidates (smaller subset) instead of all tracks
            for n in critical_candidates:
                t = self.tracking.tracks.get(n)
                if t and t.has_bundle:
                    critical_errors.append((n, t.average_error))
        ...
        if len(final_to_delete) > max_by_total:
            ...
            final_errors = []
            # Optimization: Iterate candidates instead of all tracks
            for n in final_to_delete:
                t = self.tracking.tracks.get(n)
                if t and t.has_bundle:
                    final_errors.append((n, t.average_error))
